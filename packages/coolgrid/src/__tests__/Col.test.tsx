...
describe('Col', () => {
  ...
  // T1.6: offset / order / auto — Bootstrap-baseline props missing until now.
  // We render and verify the Col mounts cleanly under each combination; the
  // CSS-string emission itself is covered indirectly by makeItResponsive +
  // the unistyle styles pipeline (separately tested in unistyle).
  describe('offset / order / auto', () => {
    it('accepts an `offset` prop without throwing', () => {
      render(
        <Container columns={12}>
          <Row>
            <Col size={6} offset={3}>
              <div data-testid="offset-col">Offset 3</div>
            </Col>
          </Row>
        </Container>,
        { wrapper },
      )
      expect(screen.getByTestId('offset-col')).toBeInTheDocument()
    })

    it('accepts an `order` prop without throwing', () => {
      render(
        <Container columns={12}>
          <Row>
            <Col size={6} order={2}>
              <div data-testid="ordered-col">Order 2</div>
            </Col>
          </Row>
        </Container>,
        { wrapper },
      )
      expect(screen.getByTestId('ordered-col')).toBeInTheDocument()
    })

    it('accepts an `auto` prop alongside or instead of size', () => {
      render(
        <Container columns={12}>
          <Row>
            <Col auto>
              <div data-testid="auto-col">Auto</div>
            </Col>
            <Col size={6}>
              <div data-testid="fixed-col">Fixed</div>
            </Col>
          </Row>
        </Container>,
        { wrapper },
      )
      expect(screen.getByTestId('auto-col')).toBeInTheDocument()
      expect(screen.getByTestId('fixed-col')).toBeInTheDocument()
    })

    it('accepts responsive offset/order/auto values', () => {
      render(
        <Container columns={12}>
          <Row>
            <Col
              size={{ xs: 12, md: 6 } as any}
              offset={{ xs: 0, md: 3 } as any}
              order={{ xs: 2, md: 1 } as any}
            >
              <div data-testid="resp-col">Responsive</div>
            </Col>
          </Row>
        </Container>,
        { wrapper },
      )
      expect(screen.getByTestId('resp-col')).toBeInTheDocument()
    })
  })
})
