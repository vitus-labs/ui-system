...
// PROBE: children built ONCE and reused — genuinely stable identity.
const HOISTED_CHILDREN = makeStableChildren(MOUNT_N)
...
// PROBE: same loop, but reusing ONE children array (real stable identity).
setupRoot()
flushSync(() => { currentRoot!.render(wrap(HOISTED_CHILDREN)) })
leafRenderCount = 0
for (let i = 1; i <= UPDATE_N; i++) {
  flushSync(() => { currentRoot!.render(wrap(HOISTED_CHILDREN)) })
}
console.log('PROBE fresh-children renders :', stableUpdateCount)
console.log('PROBE hoisted-children renders:', leafRenderCount)
